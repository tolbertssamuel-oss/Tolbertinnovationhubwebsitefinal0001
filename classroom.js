...
function renderIELTSDashboard() {
  ...
  for (let moduleId = 1; moduleId <= totalModules; moduleId += 1) {
    if (getIELTSModuleState(moduleId).completed) completedCount += 1;
  }
...
function removeDashboardLinksFromClassroomUI() {
  const dashboardAnchors = Array.from(document.querySelectorAll('a[href]')).filter((anchor) => {
    const href = String(anchor.getAttribute('href') || '').toLowerCase();
    const label = String(anchor.textContent || '').trim().toLowerCase();
    return href.includes('classroom-dashboard.html') || label.includes('dashboard');
  });

  dashboardAnchors.forEach((anchor) => {
    const parentParagraph = anchor.closest('p');
    if (parentParagraph) {
      const textWithoutAnchor = parentParagraph.textContent.replace(anchor.textContent || '', '').trim();
      if (!textWithoutAnchor) {
        parentParagraph.remove();
        return;
      }
    }

    const toolbar = anchor.closest('.lesson-toolbar');
    anchor.remove();
    if (toolbar && !toolbar.querySelector('a,button')) {
      toolbar.remove();
    }
  });
}
...
normalizeClassroomAccountStorage().finally(() => {
  ...
  wireTOEFLModulePage();
  removeDashboardLinksFromClassroomUI();
  wireAutoHideHeader();
});
